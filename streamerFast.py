...
class Streamer:
    ...
    def send(self, data_bytes: bytes) -> None:
        ...
        for data in self.break_string(data_bytes, data_length):
            ...
            if self.send_window > 40:
                print("Window hit")

            miliseconds = 0
            while self.send_window > 40:
                self.send_window = self.current_seq - self.last_ACK
                sleep(0.01)
                miliseconds += 1
                if miliseconds >= 25:
                    self.resend(self.current_seq)
                    miliseconds = 0
